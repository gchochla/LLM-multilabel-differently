...
def main():
    ...
    fig_prob, (ax2, ax1) = plt.subplots(
        ...
        figsize=(max_rank * len(experiment_stats) * 2, 6),
        sharey=True,
    )
    ...
    for i, (name, stats) in enumerate(experiment_stats.items()):
        ...
        # if i == 0:
        #     ax1[i].legend(
        #         handles=prob_legend_handles, loc='upper left', fontsize=12
        #     )
    ...
    ax1[0].set_ylabel("Top Probability", fontsize=18)
    ax2[0].set_ylabel("Second Probability", fontsize=18)
    ...
    fig_prob.suptitle(
        f"Probabilities for {args.dataset}",
        fontsize=20,
        y=0.98,
    )
    ...
    fig_prob.savefig(
        os.path.join(args.output, "top_probabilities.pdf"),
        bbox_inches="tight",
        dpi=300,
    )
    fig_prob.savefig(
        os.path.join(args.output, "top_probabilities.png"),
        bbox_inches="tight",
        dpi=300,
    )
